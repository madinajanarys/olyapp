...
import { ANIMAL_TIERS } from '../../data/petShop';
...
export default function PetAnimalFoodScreen({ navigation }) {
  const { coins, purchaseAnimalFood } = useApp();
  ...
  const buy = (tier) => {
    ...
    purchaseAnimalFood(tier);
    Alert.alert('Куплено', 'Еда в инвентаре. В хабе нажмите «Кормить» — откроется игра.');
  };
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top', 'left', 'right']}>
      <ScrollView contentContainerStyle={styles.scroll}>
        ...
        <Text style={styles.title}>Еда для животных</Text>
...
const styles = StyleSheet.create({
  ...
  rowPrice: { fontSize: 14, color: '#16a34a', fontWeight: '700', marginTop: 4 },
});
